...
public class VizStud {
	...
	VizStud(int x, int y, String nume, String prenume, String curs, Connection c) {
		con = c;
		frame = new JFrame("Vizualizare studenti curs " + curs);
		this.x = x;
		this.y = y;
		this.nume = nume;
		...
		JLabel sal = new JLabel("Vizualizare studenti curs " + curs);
		sal.setFont(new Font("", Font.BOLD, 20));

		JPanel mijloc = new JPanel();
		try {
			CallableStatement mySt = null;
			mySt = con.prepareCall("{call cauta_studenti(?)}");
			mySt.setString(1, curs);
			mySt.execute();
