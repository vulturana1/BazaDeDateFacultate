...
public class VizProf {
	...
	VizProf(int x, int y, String nume, String prenume, String curs, Connection c) {
		con = c;
		frame = new JFrame("Vizualizare profesor curs " + curs);
		this.x = x;
		this.y = y;
		this.nume = nume;
		...
		JLabel sal = new JLabel("Vizualizare profesor curs " + curs);
		sal.setFont(new Font("", Font.BOLD, 20));

		JPanel mijloc = new JPanel();
		try {
			CallableStatement mySt = null;
			mySt = con.prepareCall("{call cauta_curs(?)}");
